...
export class AuthService {
  ...
  private _smartbinUser: BehaviorSubject<SmartbinUser> = new BehaviorSubject<
    SmartbinUser
  >(null);
    ...
    this._user
      ...
      .subscribe((r) => {
        if (!r.exists) {
          ...
          this._user
            .set(this.crud.deepCopy(new SmartbinUser(user)))
            .then(() => {
              this._user
                .get()
                .pipe(take(1))
  ...
  updateSmartbinUser(data: any) {
    if (this._user) this._user.update(this.crud.deepCopy(data));
  }
  ...
  get isAdmin() {
    return localStorage.getItem('status') === 'Yu7yXGmZ0ZOiE5VwUz6N0iZI9452';
  }
}
